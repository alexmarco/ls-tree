...
class TestMain:
    ...
    def test_main_exclude_patterns(self, sample_tree: Path) -> None:
        ...
        with patch('sys.argv', ['ls-tree', '--exclude', '*.pyc', '--exclude-dir', '__pycache__', str(sample_tree)]):
            with patch('sys.stdout', new_callable=StringIO) as mock_stdout:
                main()
                output = mock_stdout.getvalue()
    ...
    def test_main_multiple_exclusions(self, sample_tree: Path) -> None:
        ...
        with patch('sys.argv', ['ls-tree', '-x', '*.pyc', '-x', '*.pyo', '-xd', '__pycache__', '-xd', 'node_modules', str(sample_tree)]):
            with patch('sys.stdout', new_callable=StringIO) as mock_stdout:
                main()
                output = mock_stdout.getvalue()
    ...
    def test_main_current_directory(self, sample_tree: Path) -> None:
        ...
        with patch('sys.argv', ['ls-tree']):
            with patch('sys.stdout', new_callable=StringIO) as mock_stdout:
                with patch('pathlib.Path.cwd', return_value=sample_tree):
                    main()
                    output = mock_stdout.getvalue()
    ...
    def test_main_json_with_metadata(self, sample_tree: Path) -> None:
        ...
        with patch('sys.argv', ['ls-tree', '--format', 'json', '--show-metadata', str(sample_tree)]):
            with patch('sys.stdout', new_callable=StringIO) as mock_stdout:
                main()
                output = mock_stdout.getvalue()
    ...
    def test_main_yaml_with_metadata(self, sample_tree: Path) -> None:
        ...
        with patch('sys.argv', ['ls-tree', '--format', 'yaml', '--show-metadata', str(sample_tree)]):
            with patch('sys.stdout', new_callable=StringIO) as mock_stdout:
                main()
                output = mock_stdout.getvalue()
    ...
    def test_main_flat_with_metadata(self, sample_tree: Path) -> None:
        ...
        with patch('sys.argv', ['ls-tree', '--format', 'flat', '--show-metadata', str(sample_tree)]):
            with patch('sys.stdout', new_callable=StringIO) as mock_stdout:
                main()
                output = mock_stdout.getvalue()
    ...
    def test_main_ascii_with_metadata(self, sample_tree: Path) -> None:
        ...
        with patch('sys.argv', ['ls-tree', '--format', 'ascii', '--show-metadata', str(sample_tree)]):
            with patch('sys.stdout', new_callable=StringIO) as mock_stdout:
                main()
                output = mock_stdout.getvalue()
    ...
    def test_main_combined_options(self, sample_tree: Path) -> None:
        ...
        with patch('sys.argv', ['ls-tree', '--format', 'tree', '--no-emoji', '--show-metadata', '--exclude', '*.pyc', str(sample_tree)]):
            with patch('sys.stdout', new_callable=StringIO) as mock_stdout:
                main()
                output = mock_stdout.getvalue()
